...
const RepaymentsPage = ({ onShowSnackbar }) => {
  ...
  const loadFlaggedItems = async () => {
    try {
      ...
      onShowSnackbar(`Failed to load flagged repayments: ${error.message}`, 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Paper sx={{ borderRadius: 2, boxShadow: 1, overflow: 'hidden' }}>
      <Box sx={{ p: 3, borderBottom: '1px solid #e0e0e0' }}>
        <Typography variant="h4" sx={{ fontWeight: 'bold', color: '#424242', mb: 1 }}>
          Repayment Fraud Detection
        </Typography>
        <Typography variant="body1" color="text.secondary">
          View and manage flagged repayment fraud detection results.
        </Typography>
      </Box>
      ...
      <Box sx={{ p: 3, borderTop: '1px solid #e0e0e0', bgcolor: '#fafafa' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Typography variant="h6" sx={{ fontWeight: 'bold', color: '#424242' }}>
            Flagged Repayments ({totalItems} total)
          </Typography>
          <Button
            variant="outlined"
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={6} sx={{ textAlign: 'center', py: 4 }}>
                    <Alert severity="info">No flagged repayments found.</Alert>
                  </TableCell>
                </TableRow>
              ) : (
