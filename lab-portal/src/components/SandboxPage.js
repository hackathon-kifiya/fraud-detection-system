...
const SandboxPage = ({ onShowSnackbar }) => {
  ...
  const loadFlaggedItems = async () => {
    try {
      ...
      const response = await labAPI.getSandboxData(params);
  ...
  const handleUpload = async (file) => {
    ...
    try {
      let response;
      response = await labAPI.uploadFile(formData);
  ...
  const handleDetection = async () => {
    ...
    try {
      const response = await labAPI.runSandboxTest({ type: currentDataType.key, days_back: 30 });
  ...
  const handleVerify = async (itemId, status) => {
    try {
      await labAPI.runSandboxTest({ itemId, status });
