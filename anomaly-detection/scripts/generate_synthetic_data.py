...
def generate_kyc_data(
    ...
    for i in range(n_normal, n_samples):
        ...
        try:
            ...
            # logger.error(f"Error generating anomaly KYC record {i}: {e}")
    ...
    for idx, cust_id in enumerate(customer_ids):
        ...
        if not is_anomaly:
            ...
            try:
                ...
                # logger.error(f"Error generating anomaly Business record {idx}: {e}")
                rec = {
                    "business_establishment_year": est_year,
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate synthetic training data for anomaly detection"
    )
    parser.add_argument(
        "--samples", type=int, default=10000, help="Number of customer samples"
    )
    parser.add_argument(
        "--anomaly-fraction",
        type=float,
    ...
    if not (0.0 <= args.anomaly_fraction <= 1.0):
    ...
    # Generate KYC
    kyc_df = generate_kyc_data(args.samples, args.anomaly_fraction)
    ...
    business_df = generate_business_data(customer_ids, args.anomaly_fraction)
